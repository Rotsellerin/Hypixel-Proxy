...
const hypixelFast = routes.find(route => route.id === 'hypixelfast')
...
const hypixelFastStatus = __test.serverListStatusResponse(hypixelFast, upstream, 47)
assert.equal(hypixelFastStatus.description.extra[3].text, '           ')
assert.equal(hypixelFastStatus.description.extra[4].text, 'Hypixel Fast')
assert.equal(hypixelFastStatus.description.extra[4].color, 'yellow')
assert.match(hypixelFastStatus.players.sample[0].name, /Hypixel Fast/)
